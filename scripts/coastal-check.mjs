...
d.tickVillages([]);
assert.equal(game.actors.length, actorsBefore + 1, 'sea cargo should depart across winter ice');
const winterCart = game.actors[game.actors.length - 1];
assert.equal(winterCart.boat, false);
assert.equal(winterCart.deliveryId, 'castle');
assert.ok(d.findPath(winterCart, game.castle), 'winter cart has a route to the keep');
assert.equal(winterCart.coinPayload, 2, 'village port adds two coins to the winter shipment');
game.population = 0;
...
console.log('Coastal founding, boat relay, 1.5x travel, ranger companion, and winter cargo: OK');
